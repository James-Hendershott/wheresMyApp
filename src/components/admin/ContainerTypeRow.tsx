...
export function ContainerTypeRow({
  ...
  if (!editing) {
    const hasTapered =
      type.topLength || type.topWidth || type.bottomLength || type.bottomWidth;
        ...
        <td className={cell}>
          {hasTapered ? (
            <div className="text-xs">
              Top: {type.topLength ?? "?"}×{type.topWidth ?? "?"}
              <br />
              Bottom: {type.bottomLength ?? "?"}×{type.bottomWidth ?? "?"}
              <br />
              H: {type.height ?? "?"}
            </div>
          ) : hasBox ? (
      ...
      <td colSpan={5} className="p-2">
        ...
        >
          ...
          <div className="flex items-center gap-2">
            <input
              name="iconKey"
              defaultValue={type.iconKey ?? ""}
              className={input}
              placeholder="Icon name (e.g., Package)"
            />
            <ContainerTypeIcon
              iconKey={type.iconKey ?? undefined}
          ...
          <div className="text-xs font-medium text-gray-700">
            Tapered Top/Bottom (optional):
          </div>
          <div className="grid grid-cols-4 gap-2">
            <input
              name="topLength"
              defaultValue={type.topLength ?? ""}
              className={input}
              type="number"
              min={1}
              placeholder="Top L"
            ...
            <input
              name="bottomLength"
              defaultValue={type.bottomLength ?? ""}
              className={input}
              type="number"
              min={1}
              placeholder="Bot L"
