...
export function PendingUserRow({
  ...
  const cell = "p-2";
  const approve = "rounded bg-green-600 px-3 py-1 text-xs font-medium text-white hover:bg-green-700";
  const reject = "rounded bg-red-600 px-3 py-1 text-xs font-medium text-white hover:bg-red-700";
  ...
  return (
    <tr className="border-t">
      ...
      <td className={cell}>{new Date(request.createdAt).toLocaleDateString()}</td>
      <td className={`${cell} text-right`}>
        <button className={approve} onClick={handleApprove} disabled={processing}>
          Approve
        </button>
        <button className={`${reject} ml-2`} onClick={handleReject} disabled={processing}>
          Reject
        </button>
      </td>
    </tr>
