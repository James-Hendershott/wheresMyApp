...
export function ProfileForm({ user }: { user: User }) {
  ...
  const inputClass = "w-full rounded border px-3 py-2 text-sm focus:outline-none focus:ring";
  const button = "rounded bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700";
  ...
  return (
    <form ref={formRef} action={formAction} className="space-y-4">
      ...
      <div>
        ...
        <input value={user.email || ""} disabled className={`${inputClass} bg-gray-100 cursor-not-allowed`} />
        <p className="mt-1 text-xs text-gray-500">Email cannot be changed.</p>
      </div>
      <div>
        <label className="mb-1 block text-sm font-medium">Avatar URL (optional)</label>
        <input name="image" defaultValue={user.image || ""} type="url" className={inputClass} />
      </div>
